...
#include "ndn-consumer-zipf-unique-cachetag.h"
...
NS_LOG_COMPONENT_DEFINE ("ndn.ConsumerZipfUniqueCachetag");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (ConsumerZipfUniqueCachetag);

TypeId
ConsumerZipfUniqueCachetag::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::ConsumerZipfUniqueCachetag")
    .SetGroupName ("Ndn")
    .SetParent<ConsumerCbr> ()
    .AddConstructor<ConsumerZipfUniqueCachetag> ()

    .AddAttribute ("NumberOfContents", "Number of the Contents in total",
                   StringValue ("100"),
                   MakeUintegerAccessor (&ConsumerZipfUniqueCachetag::SetNumberOfContents, &ConsumerZipfUniqueCachetag::GetNumberOfContents),
                   MakeUintegerChecker<uint32_t> ())

    .AddAttribute ("q", "parameter of improve rank",
                   StringValue ("0.7"),
                   MakeDoubleAccessor (&ConsumerZipfUniqueCachetag::SetQ, &ConsumerZipfUniqueCachetag::GetQ),
                   MakeDoubleChecker<double> ())
    
    .AddAttribute ("s", "parameter of power",
                   StringValue ("0.7"),
                   MakeDoubleAccessor (&ConsumerZipfUniqueCachetag::SetS, &ConsumerZipfUniqueCachetag::GetS),
                   MakeDoubleChecker<double> ())
    ;
...
ConsumerZipfUniqueCachetag::ConsumerZipfUniqueCachetag()
...
void
ConsumerZipfUniqueCachetag::SetNumberOfContents (uint32_t numOfContents)
{
  m_N = numOfContents;
...
uint32_t
ConsumerZipfUniqueCachetag::GetNumberOfContents () const
{
  return m_N;
}

void
ConsumerZipfUniqueCachetag::SetQ (double q)
{
  m_q = q;
...
double
ConsumerZipfUniqueCachetag::GetQ () const
{
  return m_q;
}

void
ConsumerZipfUniqueCachetag::SetS (double s)
{
  m_s = s;
  SetNumberOfContents (m_N);
}
...
double
ConsumerZipfUniqueCachetag::GetS () const
{
  return m_s;
}

void
ConsumerZipfUniqueCachetag::SendPacket() {
  if (!m_active) return;
  ...
  if (seq == std::numeric_limits<uint32_t>::max ()) //no retransmission
    {
      ...
      seq = ConsumerZipfUniqueCachetag::GetNextSeq();
  ...
  //zhi add the Cache tag to the Interest packet
  CacheID   CacheIdTag;
  CacheInfo CacheInfoTag;
  interest->GetPayload ()->AddPacketTag (CacheIdTag);
  interest->GetPayload ()->AddPacketTag (CacheInfoTag);
  //end

  m_transmittedInterests (interest, this, m_face);
  m_face->ReceiveInterest (interest);

  ConsumerZipfUniqueCachetag::ScheduleNextPacket ();
}


uint32_t
ConsumerZipfUniqueCachetag::GetNextSeq()
{
  ...
  if (m_firstTime)
    {
      m_sendEvent = Simulator::Schedule (Seconds (0.0),
                                         &ConsumerZipfUniqueCachetag::SendPacket, this);
      m_firstTime = false;
    }
