...
NS_LOG_COMPONENT_DEFINE ("ndn.ConsumerZipfMandelbrotUnique");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (ConsumerZipfMandelbrotUnique);

TypeId
ConsumerZipfMandelbrotUnique::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::ConsumerZipfMandelbrotUnique")
    .SetGroupName ("Ndn")
    .SetParent<ConsumerCbr> ()
    .AddConstructor<ConsumerZipfMandelbrotUnique> ()

    .AddAttribute ("NumberOfContents", "Number of the Contents in total",
                   StringValue ("100"),
                   MakeUintegerAccessor (&ConsumerZipfMandelbrotUnique::SetNumberOfContents, &ConsumerZipfMandelbrotUnique::GetNumberOfContents),
                   MakeUintegerChecker<uint32_t> ())

    .AddAttribute ("q", "parameter of improve rank",
                   StringValue ("0.7"),
                   MakeDoubleAccessor (&ConsumerZipfMandelbrotUnique::SetQ, &ConsumerZipfMandelbrotUnique::GetQ),
                   MakeDoubleChecker<double> ())
    
    .AddAttribute ("s", "parameter of power",
                   StringValue ("0.7"),
                   MakeDoubleAccessor (&ConsumerZipfMandelbrotUnique::SetS, &ConsumerZipfMandelbrotUnique::GetS),
                   MakeDoubleChecker<double> ())
    ;
...
ConsumerZipfMandelbrotUnique::ConsumerZipfMandelbrotUnique()
...
void
ConsumerZipfMandelbrotUnique::SetNumberOfContents (uint32_t numOfContents)
{
  m_N = numOfContents;
...
uint32_t
ConsumerZipfMandelbrotUnique::GetNumberOfContents () const
{
  return m_N;
}

void
ConsumerZipfMandelbrotUnique::SetQ (double q)
{
  m_q = q;
...
double
ConsumerZipfMandelbrotUnique::GetQ () const
{
  return m_q;
}

void
ConsumerZipfMandelbrotUnique::SetS (double s)
{
  m_s = s;
  SetNumberOfContents (m_N);
}
...
double
ConsumerZipfMandelbrotUnique::GetS () const
{
  return m_s;
}

void
ConsumerZipfMandelbrotUnique::SendPacket() {
  if (!m_active) return;
  ...
  ConsumerZipfMandelbrotUnique::ScheduleNextPacket ();
}


uint32_t
ConsumerZipfMandelbrotUnique::GetNextSeq()
{
  ...
  if (m_firstTime)
    {
      m_sendEvent = Simulator::Schedule (Seconds (0.0),
                                         &ConsumerZipfMandelbrotUnique::SendPacket, this);
      m_firstTime = false;
    }
