  ...
  , m_needSetDefaultRoutes (false)
{
  ...
  m_pstFactory.         SetTypeId ("ns3::ndn::pst::Default");  //constructer:PST object, zhi add
...
// SetPst function, use to set policy of PST, zhi add
void
StackHelper::SetPst (const std::string &pstClass,
                     const std::string &attr1, const std::string &value1,
                     const std::string &attr2, const std::string &value2,
                     const std::string &attr3, const std::string &value3,
                     const std::string &attr4, const std::string &value4)
{
  m_pstFactory.SetTypeId (pstClass);
  if (attr1 != "")
      m_pstFactory.Set (attr1, StringValue (value1));
  if (attr2 != "")
      m_pstFactory.Set (attr2, StringValue (value2));
  if (attr3 != "")
      m_pstFactory.Set (attr3, StringValue (value3));
  if (attr4 != "")
      m_pstFactory.Set (attr4, StringValue (value4));
}
...
StackHelper::Install (Ptr<Node> node) const
{
  ...
  // Create and aggregate PST, zhi add
  ndn->AggregateObject (m_pstFactory.Create<Pst> ());
