...
protected:
  ...
  /// \copydoc ForwardingStrategy::DidExhaustForwardingOptions
  virtual void
...
// zhi add
// fix the bug of neglecting the ReturnLimit operation in the DidExhaustForwardingOptions, which cause the Limit overflow
template<class Parent>
void
PerOutFaceLimits<Parent>::DidExhaustForwardingOptions (Ptr<Face> inFace,
                                                       Ptr<const Interest> interest,
                                                       Ptr<pit::Entry> pitEntry)
{
  NS_LOG_FUNCTION (this << pitEntry->GetPrefix ());
