/*
 * pop-cache-fr-stat.cc
 *
 *  Created on: 2015年12月3日
 *      Author: zhi
 */

#include "pop-cache-fr-stat.h"
...
NS_OBJECT_ENSURE_REGISTERED (Popcachefrstat);

LogComponent Popcachefrstat::g_log = LogComponent (Popcachefrstat::GetLogName ().c_str ());

std::string
Popcachefrstat::GetLogName ()
{
  return super::GetLogName ()+".Popcachefrstat";
}


TypeId
Popcachefrstat::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::fw::Popcachefrstat")
    .SetGroupName ("Ndn")
    .SetParent <super> ()
    .AddConstructor <Popcachefrstat> ()
    ;
  return tid;
}

Popcachefrstat::Popcachefrstat ()
{
}

void
Popcachefrstat::NotifyNewAggregate ()
{
  if (m_pit == 0)
    {
      m_pit = GetObject<Pit> ();
...
void
Popcachefrstat::OnInterest (Ptr<Face> inFace,
...
void
Popcachefrstat::OnData (Ptr<Face> inFace,
                            Ptr<Data> data)
{
...
bool
Popcachefrstat::UpdatePstInformation (Ptr<const Interest> interest)
{
