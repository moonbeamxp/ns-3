...
NS_OBJECT_ENSURE_REGISTERED (Popcachemid);

LogComponent Popcachemid::g_log = LogComponent (Popcachemid::GetLogName ().c_str ());

std::string
Popcachemid::GetLogName ()
{
  return super::GetLogName ()+".Popcachemid";
}


TypeId
Popcachemid::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::fw::Popcachemid")
    .SetGroupName ("Ndn")
    .SetParent <super> ()
    .AddConstructor <Popcachemid> ()
    ;
  return tid;
}

Popcachemid::Popcachemid ()
{
}

void
Popcachemid::OnInterest (Ptr<Face> inFace,
                                Ptr<Interest> interest)
{
  ...
  if (contentObject != 0)
    { 
      ...
      contentObject->GetPayload ()->PeekPacketTag (Popularity);
      ...
      if (interest->GetPayload ()->PeekPacketTag (hopCountTag))
        {
          ...
          contentObject->GetPayload ()->AddPacketTag (Popularity);
...
void
Popcachemid::OnData (Ptr<Face> inFace,
                            Ptr<Data> data)
{
