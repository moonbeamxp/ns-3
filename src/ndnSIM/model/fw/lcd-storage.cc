...
NS_OBJECT_ENSURE_REGISTERED (LcdStorage);

LogComponent LcdStorage::g_log = LogComponent (LcdStorage::GetLogName ().c_str ());

std::string
LcdStorage::GetLogName ()
{
  return super::GetLogName ()+".LcdStorage";
}


TypeId
LcdStorage::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::fw::LcdStorage")
    .SetGroupName ("Ndn")
    .SetParent <super> ()
    .AddConstructor <LcdStorage> ()
    ;
  return tid;
}

LcdStorage::LcdStorage ()
{
}
...
void
LcdStorage::OnData (Ptr<Face> inFace,
                    Ptr<Data> data)
{
