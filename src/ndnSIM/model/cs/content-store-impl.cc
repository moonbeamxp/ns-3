...
//zhi add
#include "../../utils/trie/lb-policy.h"
...
/**
 * @brief ContentStore with Least Benefit (LB) cache replacement policy
 **/
template class ContentStoreImpl<lb_policy_traits>;
...
//zhi add
NS_OBJECT_ENSURE_REGISTERED_TEMPL(ContentStoreImpl, lb_policy_traits);
...
//zhi add                                                  
typedef multi_policy_traits< boost::mpl::vector2< lb_policy_traits,
                                                  aggregate_stats_policy_traits > > LbWithCountsTraits;
...
//zhi add
template class ContentStoreImpl<LbWithCountsTraits>;
NS_OBJECT_ENSURE_REGISTERED_TEMPL(ContentStoreImpl, LbWithCountsTraits);
 ...
 * \brief Content Store implementing Least Frequently Used cache replacement policy
 */
class Lb : public ContentStoreImpl<lb_policy_traits> { };
#endif
