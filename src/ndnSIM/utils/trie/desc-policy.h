...
#ifndef DESC_POLICY_H_
#define DESC_POLICY_H_
...
/**
 * @brief Traits for DESC replacement policy
 */
struct desc_policy_traits
{
  /// @brief Name that can be used to identify the policy (for NS-3 object model and logging)
  static std::string GetName () { return "Desc"; }
  ...
  struct policy
  {
    ...
    typedef boost::intrusive::multiset< Container,
                                   boost::intrusive::compare< MemberHookMore< Container > >,
    ...
    public:
      ...
      update (typename parent_trie::iterator item)
      {
        ...
        get_order (item) = item->payload ()->GetIncoming (); //get request rate from outside, zhi add
      ...
      {
        get_order (item) = item->payload ()->GetIncoming ();
