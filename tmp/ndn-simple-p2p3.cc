...
main (int argc, char *argv[])
{
  ...
  ApplicationContainer Flash_Crowd1;
  
  Flash_Crowd1 = consumerHelper.Install (nodes.Get (0));
  Flash_Crowd1.Start(Seconds(0));
  Flash_Crowd1.Stop(Seconds(15));
  
  ndn::AppHelper consumerHelper1 ("ns3::ndn::ConsumerZipfMandelbrot::ConsumerPeer");
  consumerHelper1.SetAttribute ("NumberOfContents", StringValue ("10"));
  consumerHelper1.SetAttribute ("q", StringValue ("0"));
  consumerHelper1.SetAttribute ("s", StringValue ("1"));
  // Consumer will request /prefix/0, /prefix/1, ...
  consumerHelper1.SetPrefix ("/prefix");
  consumerHelper1.SetAttribute ("Frequency", StringValue ("2")); // 10 interests a second
  ...
  //producerHelper.SetAttribute ("Freshness", TimeValue (Seconds (25)));
